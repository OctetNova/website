...
document.addEventListener("DOMContentLoaded", () => {
	...
	function initLenis() {
		...
		const SCROLL_DURATION = 1; // Essayez entre 1.2 (rapide) et 2.5 (très lent)
		...
		const WHEEL_MULTIPLIER = 2.5; // Essayez entre 0.8 (peu sensible) et 1.5 (très sensible)

		// TOUCH_MULTIPLIER : Sensibilité du touch (mobile)
		const TOUCH_MULTIPLIER = 1;
		...
		window.addEventListener("mousedown", (e) => {
			...
			const isScrollbar =
				e.clientX > document.documentElement.clientWidth - scrollbarWidth;
	...
	function loadHeader() {
        ...
        <header>
            <nav id="main_nav" role="navigation" aria-label="Navigation principale">
                ...
                <ul>
                    <li><a href="jeux.html" data-i18n="navGames" ${
											currentPage === "jeux.html" ? 'class="current-page"' : ""
										}></a></li>
                    <li><a href="a_propos.html" data-i18n="navAbout" ${
											currentPage === "a_propos.html"
												? 'class="current-page"'
												: ""
										}></a></li>
                    <li><a href="contact.html" data-i18n="navContact" ${
											currentPage === "contact.html"
												? 'class="current-page"'
												: ""
										}></a></li>
                    <li><a href="index.html" class="icon-home ${
											currentPage === "index.html" ? "current-page" : ""
            ...
            <div class="mobile-nav-panel">
                <ul>
                    <li><a href="index.html" data-i18n="footerHomeFinal" ${
											currentPage === "index.html" ? 'class="current-page"' : ""
										}></a></li>
                    <li><a href="jeux.html" data-i18n="navGames" ${
											currentPage === "jeux.html" ? 'class="current-page"' : ""
										}></a></li>
                ...
                <div class="footer-categories">
                    <div class="footer-column">
                        ...
                        <ul class="pages footer-links">
                            ...
                            <li><a href="jeux.html" data-i18n="footerGames" ${
															currentPage === "jeux.html"
																? 'class="current-page"'
																: ""
														}></a></li>
                            <li><a href="a_propos.html" data-i18n="footerAbout" ${
															currentPage === "a_propos.html"
																? 'class="current-page"'
																: ""
														}></a></li>
                            <li><a href="contact.html" data-i18n="footerContact" ${
															currentPage === "contact.html"
																? 'class="current-page"'
																: ""
														}></a></li>
	...
	function initGameDescriptions() {
		const gameDescriptions = document.querySelectorAll(
			".jeux .game-item .game-description"
		);
	...
	function handlePageTransitions() {
		...
		const transitionOverlay = document.getElementById(
	...
	function initPreloader() {
		...
		const isFirstVisit =
			sessionStorage.getItem("initialLoaderCompleted") !== "true";
	...
	function initScrollEffects() {
		...
		lenis.on("scroll", ({ scroll, limit, velocity, direction, progress }) => {
			elements.forEach((el) => {
				...
				el.style.opacity =
					opacityStart + (opacityEnd - opacityStart) * opacityProgress;
			});
		});

		console.log(
			"Effets de scroll initialisés sur",
			elements.length,
			"éléments"
		);
	}
	...
	async function initI18n() {
		...
		const fetchTranslations = async (lang) => {
			try {
				...
				if (!response.ok)
					throw new Error(`Fichier de traduction introuvable pour : ${lang}`);
				...
				try {
					...
					console.error(
						"Erreur de chargement des traductions de secours:",
						fallbackError
					);
					return {};
				}
		...
		const updateGalleryButtonText = () => {
			...
			if (toggleBtn) {
				...
				if (btnText) {
					...
					const isExpanded =
						galleryContainer &&
						galleryContainer.classList.contains("is-expanded");
	...
	function initThemeAndFont() {
		...
		const applyPreference = (key, selectElement, classList, defaultValue) => {
			...
			const valueToApply = classList.includes(savedValue)
				? savedValue
				: defaultValue;
			let newBodyClassName = body.className
				.split(" ")
			...
			if (key === "userTheme") {
				...
				let newRootClassName = root.className
					.split(" ")
	...
	function initContactForm() {
		...
		form.addEventListener("submit", (e) => {
			...
			const subject = (
				currentTranslations.formEmailSubject || "Nouveau message de {name}"
			).replace("{name}", commonData.name);
			const htmlMessage = `<div style="font-family: Arial, sans-serif; line-height: 1.6;"><h2 style="color: #0080ff;">${
				currentTranslations.formEmailTitle || "Nouveau message reçu"
			}</h2><p>${
				currentTranslations.formEmailIntro ||
				"Message reçu via le formulaire de contact."
			}</p><hr><p><strong>${
				currentTranslations.formNameLabel || "Nom"
			}:</strong> ${commonData.name}</p><p><strong>${
				currentTranslations.formEmailLabel || "Email"
			}:</strong> <a href="mailto:${commonData.email}">${
				commonData.email
			}</a></p><p><strong>${
				currentTranslations.formSubjectLabel || "Sujet"
			}:</strong> ${
				commonData.subject
			}</p><h3>Message :</h3><div style="padding: 15px; background-color: #f4f4f4; border-radius: 5px; border: 1px solid #ddd; color: #333;"><p style="margin: 0;">${
				commonData.message
			}</p></div><hr><p style="font-size: 0.9em; color: #777;">${
				currentTranslations.formEmailFooter ||
				"Email envoyé depuis octetnova.fr"
			}</p></div>`;
			const data1 = {
				...commonData,
			...
			formStatus.innerHTML =
				currentTranslations.formStatusSending || "Envoi en cours...";
			...
			Promise.all([sendForm(data1), sendForm(data2)])
				.then(async ([res1, res2]) => {
					if (res1.ok && res2.ok) {
						formStatus.innerHTML =
							currentTranslations.formStatusSuccess || "Message envoyé !";
						formStatus.className = "success";
						form.reset();
					} else {
						formStatus.innerHTML =
							currentTranslations.formStatusError || "Une erreur est survenue.";
